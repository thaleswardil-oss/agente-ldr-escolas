...
def schema_resultado_tavily():
    candidato = {
        "type": "object",
        "description": "Informação encontrada para a escola, sempre acompanhada de fonte e evidência verificável.",
        "properties": {
            "valor": {
                "type": "string",
                "description": "Valor exato encontrado na fonte, sem completar ou inferir informações ausentes.",
            },
            "tipo_fonte": {
                "type": "string",
                "description": "Categoria da fonte que comprova o valor encontrado.",
                "enum": ["site_oficial", "receita", "qsa", "google_maps", "instagram", "facebook", "outro"],
            },
            "url": {
                "type": "string",
                "description": "URL completa e acessível da página em que a informação foi encontrada.",
            },
            "evidencia": {
                "type": "string",
                "description": "Trecho textual da fonte que comprova diretamente o valor informado.",
            },
            "confianca": {
                "type": "number",
                "description": "Confiança de 0 a 100 de que o dado pertence exatamente à escola e unidade pesquisadas.",
            },
        },
    ...
    lista = {
        "type": "array",
        "description": "Lista de candidatos encontrados, cada um com URL e evidência verificável.",
        "items": candidato,
    }
    return {
        "properties": {
            "identidade_confirmada": {
                "type": "boolean",
                "description": "Verdadeiro somente quando nome, município, UF e pelo menos outro identificador confirmam a mesma escola e unidade.",
            },
            "nome_oficial": {
                "type": "string",
                "description": "Nome oficial da escola confirmado pelas fontes; use o nome pesquisado quando não houver confirmação melhor.",
            },
            "municipio_confirmado": {
                "type": "string",
                "description": "Município confirmado pelas fontes para a escola pesquisada.",
            },
            "uf_confirmada": {
                "type": "string",
                "description": "Sigla da UF confirmada pelas fontes para a escola pesquisada.",
            },
            "justificativa_identidade": {
                "type": "string",
                "description": "Explicação objetiva dos sinais usados para confirmar ou rejeitar a identidade da escola.",
            },
            "razao_social_candidatos": {
                **lista,
                "description": "Razões sociais encontradas para a mesma escola ou mantenedora, com fonte e evidência.",
            },
            "cnpj_candidatos": {
                **lista,
                "description": "CNPJs encontrados para a mesma escola ou mantenedora, com fonte e evidência.",
            },
            "site_candidatos": {
                **lista,
                "description": "Sites oficiais encontrados e confirmados para a escola pesquisada.",
            },
            "diretor_candidatos": {
                **lista,
                "description": "Nomes de diretor, diretora, gestor ou responsável institucional encontrados em fontes verificáveis.",
            },
            "socio_mantenedor_candidatos": {
                **lista,
                "description": "Nomes de sócios, administradores, mantenedores ou representantes legais encontrados em CNPJ, QSA ou fontes oficiais.",
            },
            "telefone_alternativo_candidatos": {
                **lista,
                "description": "Telefones ou WhatsApp pertencentes exatamente à escola pesquisada, com fonte e evidência.",
            },
            "email_candidatos": {
                **lista,
                "description": "E-mails pertencentes exatamente à escola pesquisada, com fonte e evidência.",
            },
            "sge_atual_candidatos": {
                **lista,
                "description": "Sistemas de gestão escolar identificados por portal, login, subdomínio, aplicativo, política de privacidade ou menção oficial explícita.",
            },
            "agenda_digital_candidatos": {
                **lista,
                "description": "Aplicativos ou plataformas de agenda e comunicação escolar identificados por evidência explícita em fonte oficial.",
            },
            "observacoes": {
                "type": "string",
                "description": "Observações comerciais objetivas baseadas apenas nas fontes encontradas.",
            },
            "observacoes_tecnologia": {
                "type": "string",
                "description": "Resumo objetivo das evidências de SGE, portal do aluno, aplicativo e agenda digital.",
            },
            "conflitos": {
                "type": "array",
                "description": "Lista de divergências entre fontes, homônimos, unidades ou dados incompatíveis.",
                "items": {
                    "type": "string",
                    "description": "Descrição objetiva de um conflito encontrado entre as fontes.",
                },
            },
        },
        "required": [
...
def construir_query(dados, objetivo):
    ...
    return {
        ...
        "endereco": f'{base} endereço completo unidade rua número bairro escola oficial',
        "portal": f'{base} portal do aluno login aplicativo pais responsáveis agenda comunicação online',
    }[objetivo]
...
def step_1_pesquisa(dados, tavily, modelo_research, logger):
    ...
    respostas_busca = {}
    erros = []
    for objetivo in ["identidade", "governanca", "contatos", "tecnologia", "redes", "endereco", "portal"]:
    ...
    try:
        ...
        logger.info("STEP 1", "Pesquisa complementar habilitada para campos ausentes.")
...
def filtrar_candidatos_com_prova(candidatos, fontes, logger, step, campo):
    ...
    for candidato in preparar_candidatos(candidatos):
        ...
        tipo_fonte = classificar_fonte(url, item.get("tipo_fonte", ""))
        dominio_oficial = tipo_fonte in {"site_oficial", "receita", "qsa", "google_maps"}
        if len(valor_limpo) >= 5 and valor_limpo not in evidencia_limpa and not dominio_oficial:
            logger.alerta(step, f"Candidato de {campo} descartado: evidência insuficiente.", serializar_json(item))
            continue
        validos.append(item)
    return validos
...
def step_3_extracao(dados, pesquisa, identidade, logger):
    ...
    if not identidade.get("identidade_confirmada"):
        return {"diretor": VALOR_NAO_IDENTIFICADO, "telefone_alternativo": VALOR_NAO_IDENTIFICADO, "email": VALOR_NAO_IDENTIFICADO, "endereco": VALOR_NAO_IDENTIFICADO, "observacoes": VALOR_NAO_IDENTIFICADO, "selecoes": {}}
    ...
    enderecos = filtrar_candidatos_com_prova(extrair_lista(objeto, "endereco"), fontes, logger, "STEP 3", "endereço")
    ...
    endereco, endereco_escolhido = selecionar_candidato(enderecos, normalizar_valor_generico, logger, "STEP 3", "endereço")
    ...
    return {"diretor": diretor, "telefone_alternativo": telefone, "email": email, "endereco": endereco, "observacoes": observacoes, "selecoes": {"diretor": diretor_escolhido, "telefone_alternativo": telefone_escolhido, "email": email_escolhido, "endereco": endereco_escolhido, "socio_mantenedor": socios}}
...
def pipeline_ldr(
    ...
    resultado = {
        ...
        "endereco": extracao.get("endereco", VALOR_NAO_IDENTIFICADO),
...
with tab3:
    if "df_final" in st.session_state:
        ...
        if st.button("🚀 Processar", disabled=df_p.empty):
            try:
                rid = criar_rodada(
                    supabase,
